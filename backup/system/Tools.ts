...
class Tools {
    ...
    //初始化配置
    async InitConfig() {
      let resolve = await this.ReadConfig();
      ...
      http.listen(this.global.web_port, () => {
        console.log(`_______________________________________\n`);
        console.log(`  ${this.Curentyyyymmdd()}${this.CurentTime()} 启动完毕，访问 127.0.0.1:${this.global.web_port} 即可进入web端  \n`);
      });
    }
    ...
    app_start() {
        ...
        app.get("/profile", (req, res) => {
          ...
          res.sendFile(process.cwd() + this.global.html);
        });
