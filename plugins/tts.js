...
async function TTS(tex) {
  ...
  axios({
    ...
    data: {
       ...
       * 1-12 声线,1=2:普通男性,3:有情感的播音男性,4:有情感的萝莉声线-度丫丫;5:普通女性,6:抑扬顿挫有情感的讲故事男性(纪录频道),7:有情感的广东话女性,8:语气平淡的念诗男性(葛平),9:速读普通男性,10:略有情感的刚成年男性,11:刺耳而略有情感的讲故事男性(情感强度比6弱),12:温柔的有情感的讲故事女性
       */
      spd: "7",  //语速，取值0-15，默认为5中语速
      pit: "10", //音调，取值0-15，默认为5中语调
      vol: "10", //音量，取值0-9，默认为5中音量
      aue: "3",  //格式, 3：mp3(default) 4： pcm-16k 5： pcm-8k 6. wav
      tex: encodeURI(tex)  //如只encodeURI一次，会出现如“+”等特殊字符不能正确发音的问题
    },
  })
    .then(async function (response) {
      ...
      console.log(body);
      if (body.data) {
        console.log(`${tex} 的小夜语音合成成功`.log);
        const base64Data = body.data.replace("data:audio/x-mpeg;base64,", "");
