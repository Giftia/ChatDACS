module.exports = {
  ...
  execute: async function (msg, userId, userName, groupId, groupName, options) {
    ...
    if (isHollyHandGrenade) {
      ...
      axios.get(`http://${ONE_BOT_API_URL}/set_group_whole_ban?group_id=${groupId}&enable=1`)
      return {
        type: 'text',
        content:
    ...
    // 是常规手雷
      ...
      axios.get(`http://${ONE_BOT_API_URL}/set_group_ban?group_id=${groupId}&user_id=${target}&duration=${boomTime}`)
...
async function ReadConfig() {
  return await yaml.parse(fs.readFileSync(path.join(process.cwd(), 'config', 'config.yml'), 'utf-8'))
}
