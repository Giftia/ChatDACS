module.exports = {
  ...
  版本: "2.0",
  作者: "Giftina",
  描述: "仅在qq端生效。向被害者丢出一个手雷，一位幸运玩家将会被炸伤 0 到 120 秒（禁言）。有千分之一的概率手雷会转化为神圣手雷（Holly Hand Grenade），将会炸伤所有无辜群友（全体禁言）。手雷有60%几率成功丢出，40%几率被自己炸伤。玩家当日投掷次数大于3次后，每投掷次数增加1，自雷概率增加10%，当日有效。",
  使用示例: "一个手雷 @被害者",
  预期返回: "[被害者和投弹者概率被禁言]",
  ...
  execute: async function (msg, userId, userName, groupId, groupName, options) {
    ...
    /**
     * 手雷成功概率，60%几率成功丢出，40%几率被自己炸伤。玩家当日投掷次数大于3次后，每投掷次数增加1，自雷概率增加10%
     */
    const successfullyThrown = (Math.floor(Math.random() * 100) + (userPlayedTimes <= 3 ? 0 : userPlayedTimes) * 10) < 60;
