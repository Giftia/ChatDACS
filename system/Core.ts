...
class Core {
  ...
  init(global: Global.Global, tools: Tools.Tools) {
    this.global = global;
    this.tools = tools;
  }
  ...
  start() {
    // 306
    this.http.listen(this.global.web_port, () => {
      console.log(`_______________________________________\n`);
      console.log(`  ${this.tools.Curentyyyymmdd()}${this.tools.CurentTime()} 启动完毕，访问 127.0.0.1:${this.global.web_port} 即可进入web端  \n`);
    });
    ...
    this.io.on("connection", (socket) => {
      ...
      socket.emit("version", this.global.version);
      this.io.emit("onlineusers", ++this.global.onlineusers);
      ...
      this.tools.GetUserData(CID)
        .then(([nickname, logintimes, lastlogintime]) => {
          ...
          this.io.emit("system message", `@欢迎回来，${socket.username}(${CID}) 。这是你第${logintimes}次访问。上次访问时间：${lastlogintime}`);
        })
        ...
        .catch((reject) => {
          ...
          this.tools.RandomNickname()
            .then((resolve) => {
              ...
              this.io.emit("system message", `@新用户 ${CID} 已连接。小夜帮你取了一个随机昵称：「${socket.username}」，请前往 更多-设置 来更改昵称`);
              socket.emit("chat message", {
                CID: "0",
                msg: this.global.help,
              });
            })
            .catch((reject) => {
              ...
              socket.username = "匿名";
              this.io.emit("system message", `@新用户 ${CID} 已连接。现在你的昵称是 匿名 噢，请前往 更多-设置 来更改昵称`);
              socket.emit("chat message", {
                CID: "0",
                msg: this.global.help,
              });
      ...
      socket.on("disconnect", () => {
        this.global.onlineusers--;
        this.io.emit("onlineusers", this.global.onlineusers);
        console.log(`${this.tools.Curentyyyymmdd()}${this.tools.CurentTime()} 用户 ${socket.username} 已断开连接`);
        this.io.emit("system message", "@用户 " + socket.username + " 已断开连接");
      });

      socket.on("typing", () => {
        this.io.emit("typing", `${socket.username} 正在输入...`);
      });
      ...
      socket.on("getupdatelog", () => {
        socket.emit("updatelog", this.global.updatelog);
      });

      //致谢列表
      socket.on("thanks", () => {
        socket.emit("thanks", this.global.thanks);
      });
      ...
      socket.on("chat message", (msg) => {
        ...
        this.io.emit("chat message", { CID: CID, name: socket.username, msg: msg }); //用户广播

        //开始if地狱
        if (this.global.rename_reg.test(msg)) {
          ...
          db.all("SELECT yyyymmdd, COUNT(*) As count FROM messages Group by yyyymmdd", (e, sql) => {
            ...
            if (!e) {
              ...
              console.log(data);
              this.io.emit("chart message", data);
            } else {
              console.log(`/log_view错误：${e}`);
              this.io.emit("chat message", { CID: "0", msg: `@${e}` });
            }
          });
        } else if (this.global.bv2av_reg.test(msg)) {
          msg = msg.replace(" ", "");
          this.tools.Bv2Av(msg)
            .then((resolve) => {
              this.io.emit("chat message", { CID: "0", msg: resolve });
            })
            .catch((reject) => {
              console.log(`Bv2Av(): rejected, and err:${reject}`);
              this.io.emit("system message", `@Bv2Av() err:${reject}`);
            });
        } else if (msg === "/reload") {
          this.io.emit("reload");
        } else if (msg === "/帮助") {
          this.io.emit("chat message", { CID: "0", msg: `@${this.global.help}` });
        } else if (msg === "/随机cos") {
          this.tools.RandomCos()
            .then((resolve) => {
              this.io.emit("pic message", resolve);
            })
            .catch((reject) => {
              console.log(`RandomCos(): rejected, and err:${reject}`);
              this.io.emit("system message", `@RandomCos() err:${reject}`);
            });
        } else if (msg === "/随机买家秀") {
          this.tools.RandomTbshow()
            ...
            .catch((reject) => {
              ...
              this.io.emit("system message", `@RandomTbshow() err:${reject}`);
            });
        } else if (msg === "/随机冷知识") {
          this.tools.RandomHomeword()
            ...
            .catch((reject) => {
              ...
              this.io.emit("system message", `@RandomHomeword() err:${reject}`);
            });
        } else if (msg === "/随机二次元图") {
          this.tools.RandomECY()
            ...
            .catch((reject) => {
              ...
              this.io.emit("system message", `@RandomECY() err:${reject}`);
            });
        } //吠
        else if (this.global.yap_reg.test(msg)) {
          msg = msg.replace("/吠 ", "");
          msg = msg.replace("/吠", "");
          this.tools.BetterTTS(msg)
            .then((resolve) => {
              this.io.emit("audio message", resolve);
            })
            .catch((reject) => {
              console.log(`TTS错误：${reject}`);
              this.io.emit("system message", `@TTS错误：${reject}`);
            });
        } //教学系统，抄板于虹原翼版小夜v3
        else if (this.global.teach_reg.test(msg)) {
          msg = msg.substr(2).split("答：");
          if (msg.length !== 2) {
            console.log(`教学指令：分割有误，退出教学`);
            this.io.emit("system message", `@你教的姿势不对噢qwq`);
            return 0;
          }
          let ask = msg[0].trim(),
            ans = msg[1].trim();
          ...
          if (ask.indexOf(/\r?\n/g) !== -1) {
            ...
            this.io.emit("system message", `@关键词不能换行啦qwq`);
            return 0;
          }
          ...
          for (let i in this.global.black_list_words) {
            if (
              ...
              this.io.emit("system message", `@你教的内容里有主人不允许小夜学习的词qwq`);
              return 0;
            }
          }
          if (Buffer.from(ask).length < 4) {
            ...
            this.io.emit("system message", `@关键词太短了啦qwq，至少要4个字节啦`);
            return 0;
          }
          if (ask.length > 350 || ans.length > 350) {
            //图片长度差不多是350左右
            console.log(`教学指令：教的太长了，退出教学`);
            this.io.emit("system message", `@你教的内容太长了，小夜要坏掉了qwq，不要呀`);
            return 0;
          }
          ...
          this.io.emit("system message", `@哇！小夜学会啦！对我说：${ask} 试试吧，小夜有可能会回复 ${ans} 噢`);
          return 0;
        } else {
  ...
  start_qqbot() {
    this.app.post(this.global.go_cqhttp_service, (req, res) => {
      //禁言1小时以上自动退群
    ...
    /批准 ${req.body.flag}`;
      ...
      console.log(notify);
      this.io.emit("system message", `@${notify}`);
      ...
      if (this.global.isVideo_reg.test(req.body.message)) {
        ...
        this.io.emit("qqvideo message", { file: url, filename: "qq视频" });
        res.send();
        return 0;
      }
                      ...
                      if (sql[0].loop_bomb_enabled === 0 || 60 - process.hrtime([sql[0].loop_bomb_start_time, 0])[0] < 0) {
                        ...
                        this.tools.ECYWenDa()
                          .then((resolve) => {
                            ...
                            db.run(
                              `UPDATE qq_group SET loop_bomb_answer = '${answer}', loop_bomb_onwer = '${req.body.user_id}' , loop_bomb_start_time = '${process.hrtime()[0]
                        ...
                        (this.global.boom_timer = setTimeout(function () {
                          ...
                          db.all(`SELECT * FROM qq_group WHERE group_id = '${req.body.group_id}'`, (err, sql) => {
                            if (!err && sql[0]) {
                              ...
                              this.io.emit("system message", `@群 ${req.body.group_id} 的击鼓传雷到达时间，炸了`);
                            }
                          });
                        }, 1000 * 60));
                                      ...
                                      if (!err && sql[0]) {
                                        this.tools.ECYWenDa()
                                          .then((resolve) => {
                                            ...
                                            //金手指
                                            request(
                                              `http://${this.global.go_cqhttp_api}/set_group_card?group_id=${req.body.group_id
    ...
    `;
                          ...
                          request(
                            ...
                            function (error, _response, _body) {
                              if (!error) {
                                ...
                                this.io.emit("system message", `@qqBot小夜在群 ${resolve} 抽风了，发送了 ${prprmsg}`);
                              } else {
                                console.log(`请求${this.global.go_cqhttp_api}/send_group_msg错误：${error}`);
                              }
                            }
                ...
                if (fudu_flag < this.global.fudu_probability) {
                  ...
                  this.io.emit("system message", `@qqBot小夜复读 ${req.body.message}`);
                    ...
                    .catch((reject) => {
                      ...
                      this.tools.GetBalabalaList()
                        .then((resolve) => {
                          ...
                          this.io.emit("system message", `@qqBot小夜觉得${random_balabala}`);
                        ...
                        .catch((reject) => {
                          ...
                          this.io.emit("system message", `@qqBot小夜试图balabala但出错了：${reject}`);
                          return 0;
                        });
                    });
                } else {
    ...
    this.tools.GetLaststDanmu()
      .then((resolve) => {
        if (this.global.last_danmu_timeline === resolve.timeline) {
          ...
          this.io.emit("sysrem message", `@弹幕传来： ${resolve.text}`);
