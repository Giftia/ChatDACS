...
- **OneBot Provider**: An external or legacy protocol implementation that connects a QQ account to the QQ platform Adapter. NapCatQQ is the recommended provider; `external` keeps generic OneBot 11 compatibility and `go-cqhttp` is retained for legacy deployments.
...
- Keep QQ protocol implementations outside ChatDACS ownership. The Adapter depends on the OneBot 11 contract, while startup probes and integration guides handle provider-specific setup.
