...
exports.RainbowPi = exports.ECYWenDa = exports.WenDa = exports.QunGugua = exports.Gugua = exports.DelayAlert = exports.GetLaststDanmu = exports.BetterTTS = exports.TTS = exports.GetBalabalaList = exports.RandomGroupList = exports.SaveQQimg = exports.ChatProcess = exports.ChatJiebaFuzzy = exports.sqliteAll = exports.InitConfig = exports.ReadConfig = exports.PrprDoge = exports.RandomNickname = exports.RandomHomeword = exports.RandomECY = exports.RandomTbshow = exports.SearchTag = exports.RandomR18 = exports.RandomCos = exports.Bv2Av = exports.GetUserData = exports.Getnews = exports.sha1 = exports.CurentTime = exports.Curentyyyymmdd = void 0;
//系统配置和开关，以及固定变量
var version = "ChatDACS v3.0.20-Dev"; //版本号，会显示在浏览器tab与标题栏
var html = "/static/index.html"; //前端页面路径，old.html为旧版前端
var boom_timer; //60s计时器
var onlineusers = 0, //预定义
Tiankey, sumtkey, baidu_app_id, baidu_api_key, baidu_secret_key, last_danmu_timeline, bot_qq, black_list_words, qq_admin_list, blive_room_id, chat_swich, conn_go_cqhttp, Now_On_Live, web_port, go_cqhttp_service, go_cqhttp_api, topN, reply_probability, fudu_probability, chaos_probability, req_fuliji_list, req_ECY_list, req_no_trap_list, qqimg_to_web, max_mine_count, cos_total_count, xiaoye_ated, private_service_swich, c1c_count = 0;
//web端配置
var help = "主人你好，我是小夜。欢迎使用沙雕Ai聊天系统 ChatDACS (Chatbot : shaDiao Ai Chat System)。在这里，你可以与经过 2w+用户调教养成的人工智能机器人小夜实时聊天，它有着令人激动的、实用的在线涩图功能，还可以和在线的其他人分享你的图片、视频与文件。现在就试试使用在聊天框下方的便捷功能栏吧，功能栏往右拖动还有更多功能。";
var thanks = "致谢（排名不分先后）：https://niconi.co.ni/、https://www.layui.com/、https://lceda.cn/、https://www.dnspod.cn/、Daisy_Liu、http://blog.luckly-mjw.cn/tool-show/iconfont-preview/index.html、https://ihateregex.io/、https://www.maoken.com/、https://www.ngrok.cc/、https://uptimerobot.com/、https://shields.io/、https://ctf.bugku.com/、https://blog.squix.org/、https://hostker.com/、https://www.tianapi.com/、https://api.sumt.cn/、https://github.com/Mrs4s/go-cqhttp、https://colorhunt.co/、https://github.com/、https://gitee.com/、https://github.com/windrises/dialogue.moe、https://api.lolicon.app/、https://bww.lolicon.app/、https://iw233.cn/main.html、https://blog.csdn.net/jia20003/article/details/7228464、还有我的朋友们，以及倾心分享知识的各位";
var update_text = "\u4F18\u5316r18\u4E0E\u6765\u70B9xx\u63D0\u793A";
var updatelog = "<h1>" + version + "</h1><br/>" + update_text;
//模块依赖和底层配置
...
//正则
var rename_reg = new RegExp("^/rename [\u4e00-\u9fa5a-z0-9]{1,10}$"); //允许1-10长度的数英汉昵称
var bv2av_reg = new RegExp("^[a-zA-Z0-9]{10,12}$"); //匹配bv号
var isImage_reg = new RegExp("\\[CQ:image,file="); //匹配qqBot图片
var change_reply_probability_reg = new RegExp("^/admin_change_reply_probability [0-9]*"); //匹配修改qqBot小夜回复率
var change_fudu_probability_reg = new RegExp("^/admin_change_fudu_probability [0-9]*"); //匹配修改qqBot小夜复读率
var img_url_reg = new RegExp("https(.*term=)"); //匹配图片地址
var isVideo_reg = new RegExp("^\\[CQ:video,file="); //匹配qqBot图片
var video_url_reg = new RegExp("http(.*term=unknow)"); //匹配视频地址
var yap_reg = new RegExp("^/吠.*"); //匹配请求语音
var come_yap_reg = new RegExp("^/嘴臭(.*)"); //匹配对话语音
var teach_reg = new RegExp("^问：(.*)答：(.*)"); //匹配教学指令
var prpr_reg = new RegExp("^/prpr (.*)"); //匹配prpr
var pohai_reg = new RegExp("^/迫害 (.*)"); //匹配迫害p图
var teach_balabala_reg = new RegExp("^/说不出话 (.*)"); //匹配balabala教学
var hand_grenade_reg = new RegExp("^一个手雷(.*)"); //匹配一个手雷
var mine_reg = new RegExp("^埋地雷"); //匹配埋地雷
var fuck_mine_reg = new RegExp("^踩地雷"); //匹配踩地雷
var hope_flower_reg = new RegExp("^希望的花(.*)"); //匹配希望的花
var loop_bomb_reg = new RegExp("^击鼓传雷(.*)"); //匹配击鼓传雷
var is_qq_reg = new RegExp("^[1-9][0-9]{4,9}$"); //校验是否是合法的qq号
var has_qq_reg = new RegExp("\\[CQ:at,qq=(.*)\\]"); //匹配是否有@
var admin_reg = new RegExp("/admin (.*)"); //匹配管理员指令
var setu_reg = new RegExp(".*图.*来.*|.*来.*图.*|.*[色涩瑟].*图.*"); //匹配色图来指令
var i_have_a_friend_reg = new RegExp("我有一个朋友说.*|我有个朋友说.*"); //匹配我有个朋友指令
var open_ju = new RegExp("张菊.*"); //匹配张菊指令
var close_ju = new RegExp("闭菊.*"); //匹配闭菊指令
var feed_back = new RegExp("^/报错.*"); //匹配报错指令
var ascii_draw = new RegExp("/字符画.*"); //匹配字符画指令
var gugua = new RegExp("^/孤寡.*"); //匹配孤寡指令
var cp_story = new RegExp("^/cp.*|^cp.*"); //匹配cp文指令
var fake_forward = new RegExp("^/强制迫害.*"); //匹配伪造转发指令
var approve_group_invite = new RegExp("^/批准 (.*)"); //匹配批准加群指令
var make_qrcode = new RegExp("^qr (.*)"); //匹配生成二维码指令
var come_some = new RegExp("^来点(.*)"); //匹配来点xx指令
var bww_reg = new RegExp("^/黑白图 (.*)"); //匹配黑白图
//声明TTS调用接口
var SpeechClient;
//qqBot小夜核心代码，对接go-cqhttp
function start_qqbot() {
    app.post(go_cqhttp_service, function (req, res) {
        //禁言1小时以上自动退群
        ...
        if (req.body.message_type == "private" && req.body.user_id == qq_admin_list[0] && approve_group_invite.test(req.body.message)) {
            var flag_1 = req.body.message.match(approve_group_invite)[1];
            request("http://" + go_cqhttp_api + "/set_group_add_request?flag=" + encodeURI(flag_1) + "&type=invite&approve=1", function (error, _response, _body) {
                if (!error) {
                    console.log(("\u6279\u51C6\u4E86\u8BF7\u6C42id " + flag_1).log);
                    res.send({ reply: "\u5DF2\u6279\u51C6" });
                }
                else {
        ...
        if (qqimg_to_web) {
            if (isImage_reg.test(req.body.message)) {
                var url = img_url_reg.exec(req.body.message);
                SaveQQimg(url)
        ...
        if (isVideo_reg.test(req.body.message)) {
            var url = video_url_reg.exec(req.body.message)[0];
        ...
        if (req.body.message_type == "group" ||
            ...
            if (open_ju.test(req.body.message) && has_qq_reg.test(req.body.message)) {
                ...
                if (is_qq_reg.test(who)) {
                    //如果是自己要被张菊，那么张菊
                    if (bot_qq == who) {
                        request("http://" + go_cqhttp_api + "/get_group_member_info?group_id=" + req.body.group_id + "&user_id=" + req.body.user_id, function (_error, _response, body) {
                    ...
                    if (!err && sql[0]) {
                        ...
                        if (sql[0].talk_enabled === 0) {
                            ...
                            if (feed_back.test(req.body.message)) {
                                ...
                                request("http://" + go_cqhttp_api + "/send_group_msg?group_id=120243247&message=" + encodeURI(msg_1), function (error, _response, _body) {
                                    if (!error) {
                                        console.log((req.body.user_id + " \u53CD\u9988\u4E86\u9519\u8BEF " + msg_1).log);
                                    }
                                    else {
                                        console.log("\u8BF7\u6C42" + go_cqhttp_api + "/send_group_msg\u9519\u8BEF\uFF1A" + error);
                                    }
                                });
                                request("http://" + go_cqhttp_api + "/send_group_msg?group_id=474164508&message=" + encodeURI(msg_1), function (error, _response, _body) {
                                    if (error) {
                                        console.log("\u8BF7\u6C42" + go_cqhttp_api + "/send_group_msg\u9519\u8BEF\uFF1A" + error);
                                    }
                                });
                            ...
                            if (req.body.sub_type === "poke" && req.body.target_id == bot_qq) {
                                ...
                                if (c1c_count > 2) {
                                    ...
                                    request("http://" + go_cqhttp_api + "/send_group_msg?group_id=" + req.body.group_id + "&message=" + encodeURI(final), function (error, _response, _body) {
                                        if (!error) {
                                            console.log((req.body.user_id + " \u6233\u4E86\u4E00\u4E0B " + req.body.target_id).log);
                                            request("http://" + go_cqhttp_api + "/set_group_ban?group_id=" + req.body.group_id + "&user_id=" + req.body.user_id + "&duration=10", function (error, _response, _body) {
                                                if (!error) {
                            ...
                            if (teach_reg.test(req.body.message)) {
                                ...
                                for (var i in black_list_words) {
                                    if (ask.toLowerCase().indexOf(black_list_words[i].toLowerCase()) !== -1 ||
                            ...
                            if (teach_balabala_reg.test(req.body.message)) {
                                ...
                                msg_3 = msg_3.replace("/说不出话", "");
                                console.log((req.body.user_id + "(" + req.body.sender.nickname + ") \u60F3\u8981\u6559\u7ED9\u5C0F\u591Cbalabala\uFF1A" + msg_3 + "\uFF0C\u73B0\u5728\u5F00\u59CB\u68C0\u6D4B\u5408\u6CD5\u6027").log);
                                for (var i in black_list_words) {
                                    if (msg_3.toLowerCase().indexOf(black_list_words[i].toLowerCase()) !== -1 ||
                                        msg_3.toLowerCase().indexOf(black_list_words[i].toLowerCase()) !== -1) {
                            ...
                            if (setu_reg.test(req.body.message)) {
                                RandomCos()
                                    .then(function (resolve) {
                                    var setu_file = "http://127.0.0.1:" + web_port + "/" + resolve.replace(/\//g, "\\");
                                    res.send({
                            ...
                            if (req.body.message == "r18") {
                                ...
                                RandomR18()
                                    ...
                                    request("http://" + go_cqhttp_api + "/send_group_msg?group_id=" + req.body.group_id + "&message=" + encodeURI("[CQ:image,file=" + setu_file + ",url=" + setu_file + "]"), function (error, _response, _body) {
                                        if (error) {
                                            console.log("\u8BF7\u6C42" + go_cqhttp_api + "/send_group_msg\u9519\u8BEF\uFF1A" + error);
                                        }
                                    });
                                    ...
                                    request("http://" + go_cqhttp_api + "/send_group_msg?group_id=" + req.body.group_id + "&message=" + encodeURI("\u4F60\u8981\u7684r18\u53D1\u9001\u5931\u8D25\u5566\uFF1A" + reject), function (error, _response, _body) {
                                        if (error) {
                                            console.log("\u8BF7\u6C42" + go_cqhttp_api + "/send_group_msg\u9519\u8BEF\uFF1A" + error);
                                        }
                                    });
                            ...
                            if (come_some.test(req.body.message)) {
                                var tag_1 = req.body.message.match(come_some)[1];
                                ...
                                SearchTag(tag_1)
                                    ...
                                    request("http://" + go_cqhttp_api + "/send_group_msg?group_id=" + req.body.group_id + "&message=" + encodeURI("\u4F60\u8981\u7684" + tag_1 + "\u53D1\u9001\u5931\u8D25\u5566\uFF1A" + reject), function (error, _response, _body) {
                                        if (error) {
                                            console.log("\u8BF7\u6C42" + go_cqhttp_api + "/send_group_msg\u9519\u8BEF\uFF1A" + error);
                                        }
                                    });
                            ...
                            if (yap_reg.test(req.body.message)) {
                                ...
                                BetterTTS(tex)
                                    .then(function (resolve) {
                                    var tts_file = "[CQ:record,file=http://127.0.0.1:" + web_port + resolve.file + ",url=http://127.0.0.1:" + web_port + resolve.file + "]";
                            ...
                            if (fake_forward.test(req.body.message)) {
                                ...
                                if (req.body.message == "/强制迫害") {
                                    ...
                                    who_3 = who_3.replace("]", "");
                                    who_3 = who_3.trim();
                                    if (is_qq_reg.test(who_3)) {
                                        console.log(("\u7FA4 " + req.body.group_id + " \u7684 \u7FA4\u5458 " + req.body.user_id + " \u5F3A\u5236\u8FEB\u5BB3 " + who_3).log);
                                    }
                                    else {
                                        //目标不是qq号
                                        who_3 = req.body.sender.user_id; //如果没有要求迫害谁，那就是迫害自己
                                ...
                                //发送
                                //先获取昵称
                                request("http://" + go_cqhttp_api + "/get_group_member_info?group_id=" + req.body.group_id + "&user_id=" + who_3 + "&no_cache=0", function (error, _response, body) {
                                    if (!error) {
                                        body = JSON.parse(body);
                            ...
                            if (pohai_reg.test(req.body.message)) {
                                ...
                                //先搜索被迫害人是否在迫害名单里
                                for (var i in pohai_list) {
                                    if (pohai_who === pohai_list[i]) {
                                        //被迫害人发现
                                        pohai_pic_1 = pohai_pic_list[i];
                                ...
                                if (has_qq_reg.test(pohai_tex_1)) {
                                    ...
                                    if (is_qq_reg.test(qq_id)) {
                                        //获取qq号在群内的昵称
                                        request("http://" + go_cqhttp_api + "/get_group_member_info?group_id=" + req.body.group_id + "&user_id=" + qq_id + "&no_cache=0", function (error, _response, body) {
                                            //这一步实在是太慢了啊实在不想异步了
                                            if (!error) {
                                                body = JSON.parse(body);
                                                ...
                                                if (pohai_tex_1.length > tex_config_1[3]) {
                                                    ...
                                                    var new_pohai_tex = "";
                                                    for (var i = 0, j = 1; i < pohai_tex_1.length; i++, j++) {
                                                        if (j && j % enter == 0) {
                                                            new_pohai_tex += pohai_tex_1[i] + "\n";
                                                        }
                                                        else {
                                                            new_pohai_tex += pohai_tex_1[i];
                                                        }
                                                ...
                                                loadImage(sources_1).then(function (image) {
                                                    ...
                                                    var file_online = "http://127.0.0.1:" + web_port + "/xiaoye/images/" + sha1(canvas.toBuffer()) + ".jpg";
                            ...
                            if (hand_grenade_reg.test(req.body.message)) {
                                ...
                                if (holly_hand_grenade < 10) {
                                    ...
                                    if (req.body.message === "一个手雷") {
                                        ...
                                        who_4 = who_4.replace("]", "");
                                        who_4 = who_4.trim();
                                        if (is_qq_reg.test(who_4)) {
                                            console.log(("\u7FA4 " + req.body.group_id + " \u7684 \u7FA4\u5458 " + req.body.user_id + " \u5C1D\u8BD5\u5411 " + who_4 + " \u4E22\u51FA\u4E00\u9897\u624B\u96F7").log);
                                        }
                                        else {
                                            //目标不是qq号
                                    ...
                                    if (success_flag < 50 || who_4 === req.body.user_id) {
                                        ...
                                        //成功丢出手雷
                                        request("http://" + go_cqhttp_api + "/set_group_ban?group_id=" + req.body.group_id + "&user_id=" + who_4 + "&duration=" + boom_time_1, function (error, _response, _body) {
                                            if (!error) {
                                                console.log(("\u7FA4 " + req.body.group_id + " \u7684 \u7FA4\u5458 " + req.body.user_id + " \u7684\u624B\u96F7\u70B8\u5230\u4E86 " + who_4).log);
                                                res.send({
                            ...
                            if (hope_flower_reg.test(req.body.message)) {
                                ...
                                if (req.body.message === "希望的花") {
                                    ...
                                    who_5 = who_5.replace("]", "");
                                    who_5 = who_5.trim();
                                    if (is_qq_reg.test(who_5)) {
                                        console.log(("\u7FA4 " + req.body.group_id + " \u7684 \u7FA4\u5458 " + req.body.user_id + " \u5411 " + who_5 + " \u4E22\u51FA\u4E00\u6735\u5E0C\u671B\u7684\u82B1").log);
                                    }
                                    else {
                                        //目标不是qq号
                                ...
                                //先救活目标
                                request("http://" + go_cqhttp_api + "/set_group_ban?group_id=" + req.body.group_id + "&user_id=" + who_5 + "&duration=0", function (error, _response, _body) {
                                    if (!error) {
                                        console.log(("\u7FA4 " + req.body.group_id + " \u7684 \u7FA4\u5458 " + req.body.user_id + " \u6551\u6D3B\u4E86 " + who_5).log);
                                        res.send({
                                ...
                                //再禁言团长
                                request("http://" + go_cqhttp_api + "/set_group_ban?group_id=" + req.body.group_id + "&user_id=" + req.body.user_id + "&duration=" + boom_time_2, function (error, _response, _body) {
                                    if (!error) {
                                        console.log((req.body.user_id + " \u81EA\u5DF1\u88AB\u70B8\u4F24" + boom_time_2 + "\u79D2").log);
                                    }
                                    else {
                                        console.log("\u8BF7\u6C42" + go_cqhttp_api + "/set_group_whole_ban\u9519\u8BEF\uFF1A" + error);
                                        res.send({ reply: "\u65E5\u5FD2\u5A18\uFF0C\u600E\u4E48\u53C8\u51FA\u9519\u4E86" });
                                ...
                                db.all("SELECT * FROM qq_group WHERE group_id = '" + req.body.group_id + "'", function (err, sql) {
                                    if (!err && sql[0]) {
                                        ...
                                        if (sql[0].loop_bomb_enabled === 0 || 60 - process.hrtime([sql[0].loop_bomb_start_time, 0])[0] < 0) {
                                            ...
                                            request("http://" + go_cqhttp_api + "/send_group_msg?group_id=" + req.body.group_id + "&message=" + encodeURI(text), function (error, _response, _body) {
                                                if (!error) {
                                            ...
                                            ECYWenDa()
                                                ...
                                                //金手指
                                                request("http://" + go_cqhttp_api + "/set_group_card?group_id=" + req.body.group_id + "&user_id=" + req.body.user_id + "&card=" + encodeURI(answer), function (error, _response, _body) {
                                                    if (!error) {
                                                ...
                                                //丢出问题
                                                setTimeout(function () {
                                                    request("http://" + go_cqhttp_api + "/send_group_msg?group_id=" + req.body.group_id + "&message=" + encodeURI(question), function (error, _response, _body) {
                                                        if (!error) {
                                                        }
                                                        else {
                                                            console.log("\u8BF7\u6C42" + go_cqhttp_api + "/send_group_msg\u9519\u8BEF\uFF1A" + error);
                                            ...
                                            boom_timer = setTimeout(function () {
                                                ...
                                                db.all("SELECT * FROM qq_group WHERE group_id = '" + req.body.group_id + "'", function (err, sql) {
                                                    if (!err && sql[0]) {
                                                        request("http://" + go_cqhttp_api + "/set_group_ban?group_id=" + req.body.group_id + "&user_id=" + sql[0].loop_bomb_onwer + "&duration=" + boom_time, function (error, _response, _body) {
                                                            if (!error) {
                                                                console.log((sql[0].loop_bomb_onwer + " \u5728\u7FA4 " + req.body.group_id + " \u56DE\u7B54\u8D85\u65F6\uFF0C\u88AB\u70B8\u4F24" + boom_time + "\u79D2").log);
                                                                //金手指关闭
                                                                request("http://" + go_cqhttp_api + "/set_group_card?group_id=" + req.body.group_id + "&user_id=" + sql[0].loop_bomb_onwer + "&card=", function (error, _response, _body) {
                                                                    if (!error) {
                                                                ...
                                                                request("http://" + go_cqhttp_api + "/send_group_msg?group_id=" + req.body.group_id + "&message=" + encodeURI(end), function (error, _response, _body) {
                                                                    if (!error) {
                                            ...
                                            db.all("SELECT * FROM qq_group WHERE group_id = '" + req.body.group_id + "'", function (err, sql) {
                                                if (!err && sql[0]) {
                                                    ...
                                                    if (sql[0].loop_bomb_answer == your_answer) {
                                                        ...
                                                        if (sql[0].loop_bomb_onwer != req.body.user_id) {
                                                            ...
                                                            request("http://" + go_cqhttp_api + "/send_group_msg?group_id=" + req.body.group_id + "&message=" + encodeURI(end), function (error, _response, _body) {
                                                                if (!error) {
                                                                    ...
                                                                    //金手指关闭
                                                                    request("http://" + go_cqhttp_api + "/set_group_card?group_id=" + req.body.group_id + "&user_id=" + sql[0].loop_bomb_onwer + "&card=", //req.body.user_id
                                                                    ...
                                                                    //禁言
                                                                    request("http://" + go_cqhttp_api + "/set_group_ban?group_id=" + req.body.group_id + "&user_id=" + req.body.user_id + "&duration=60", function (error, _response, _body) {
                                                                        if (!error) {
                                                            ...
                                                            //回答正确
                                                            //金手指关闭
                                                            request("http://" + go_cqhttp_api + "/set_group_card?group_id=" + req.body.group_id + "&user_id=" + req.body.user_id + "&card=", function (error, _response, _body) {
                                                                if (!error) {
                                                                    console.log("\u51FB\u9F13\u4F20\u96F7\u91D1\u624B\u6307\u5DF2\u542F\u52A8".log);
                                                                }
                                                                else {
                                                        ...
                                                        setTimeout(function () {
                                                            request("http://" + go_cqhttp_api + "/get_group_member_list?group_id=" + req.body.group_id, function (err, response, body) {
                                                                body = JSON.parse(body);
                                                                    ...
                                                                    db.all("SELECT * FROM qq_group WHERE group_id = '" + req.body.group_id + "'", function (err, sql) {
                                                                        if (!err && sql[0]) {
                                                                            ECYWenDa()
                                                                                ...
                                                                                //金手指
                                                                                request("http://" + go_cqhttp_api + "/set_group_card?group_id=" + req.body.group_id + "&user_id=" + rand_user_1 + "&card=" + encodeURI(answer), function (error, _response, _body) {
                                                                                    if (!error) {
                                                                                        console.log("\u51FB\u9F13\u4F20\u96F7\u91D1\u624B\u6307\u5DF2\u542F\u52A8".log);
                                                                                    }
                                                                                    else {
                                                        ...
                                                        console.log((req.body.user_id + " \u5728\u7FA4 " + req.body.group_id + " \u56DE\u7B54\u9519\u8BEF\uFF0C\u88AB\u70B8\u4F24" + boom_time_3 + "\u79D2").log);
                                                        clearTimeout(boom_timer);
                                                        request("http://" + go_cqhttp_api + "/send_group_msg?group_id=" + req.body.group_id + "&message=" + encodeURI(end), function (error, _response, _body) {
                                                            if (!error) {
                                                                io.emit("system message", "@" + sql[0].loop_bomb_onwer + " \u5728\u7FA4 " + req.body.group_id + " \u56DE\u7B54\u6B63\u786E");
                                                                //禁言
                                                                request("http://" + go_cqhttp_api + "/set_group_ban?group_id=" + req.body.group_id + "&user_id=" + req.body.user_id + "&duration=" + boom_time_3, function (error, _response, _body) {
                                                                    if (!error) {
                                                                        console.log(("\u62A2\u7B54\u4E86\uFF0C" + req.body.user_id + " \u88AB\u7981\u8A00").error);
                                                                    }
                                                                    else {
                            ...
                            if (req.body.message === "/status") {
                                ...
                                if (self_id != bot_qq) {
                                    ...
                                    var stat_1 = "\u914D\u7F6Eqq\uFF1A" + bot_qq + " \u548C\u5B9E\u9645\u767B\u5F55qq\uFF1A" + self_id + " \u4E0D\u5339\u914D\uFF0C\u81EA\u52A8\u66F4\u65B0\u4E86qq\u53F7\u4E3A " + self_id;
                                    console.log(("" + stat_1).error);
                                    bot_qq = self_id;
                                    res.send({
                                        reply: stat_1
                                    });
                            ...
                            if (gugua.test(req.body.message)) {
                                ...
                                who_6 = who_6.replace("]", "");
                                who_6 = who_6.trim();
                                if (is_qq_reg.test(who_6)) {
                                    request("http://" + go_cqhttp_api + "/get_friend_list", function (err, response, body) {
                                        body = JSON.parse(body);
                                        if (!err && body.data.length != 0) {
                                            for (var i in body.data) {
                                                if (who_6 == body.data[i].user_id) {
                                                    res.send({ reply: "\u5C0F\u591C\u6536\u5230\u4E86\u4F60\u7684\u5B64\u5BE1\u8BA2\u5355\uFF0C\u73B0\u5728\u5C31\u5F00\u59CB\u5B64\u5BE1[CQ:at,qq=" + who_6 + "]\u4E86\u5662\u5B64\u5BE1~" });
                                                    request("http://" + go_cqhttp_api + "/send_private_msg?user_id=" + who_6 + "&message=" + encodeURI("\u60A8\u597D\uFF0C\u6211\u662F\u5B64\u5BE1\u5C0F\u591C\uFF0C\u60A8\u7684\u597D\u53CB " + req.body.user_id + " \u7ED9\u60A8\u70B9\u4E86\u4E00\u4EFD\u5B64\u5BE1\u5957\u9910\uFF0C\u8BF7\u67E5\u6536"), function (error, _response, _body) {
                                                        if (!error) {
                                                            console.log(("\u7FA4 " + req.body.group_id + " \u7684 \u7FA4\u5458 " + req.body.user_id + " \u5B64\u5BE1\u4E86 " + who_6).log);
                                                        }
                                                        else {
                                                            console.log("\u8BF7\u6C42" + go_cqhttp_api + "/send_private_msg\u9519\u8BEF\uFF1A" + error);
                                                        }
                            ...
                            if (bww_reg.test(req.body.message)) {
                                var msg_8 = req.body.message + " " + " "; //结尾加2个空格防爆
                                msg_8 = msg_8.substr(4).split(" ");
                                console.log(msg_8);
                                var pic = msg_8[1].trim(), //使用图片
                                tex1_1 = msg_8[2].trim(), //使用文字1
                                tex2_1 = msg_8[3].trim(); //使用文字2
                                //如果没有使用图片的话，默认图片
                                if (!pic) {
                                    pic =
                                ...
                                //开始黑白
                                var sources_3 = img_url_reg.exec(pic)[0]; //取图片链接
                            ...
                            if (make_qrcode.test(req.body.message)) {
                                var content = req.body.message.match(make_qrcode)[1];
                                res.send({
                            ...
                            if (req.body.message === "/admin alert_open") {
                                for (var i in qq_admin_list) {
                                    if (req.body.user_id == qq_admin_list[i]) {
                                        ...
                                        AlertOpen().then(function (resolve) {
                                            res.send({
                            ...
                            if (change_reply_probability_reg.test(req.body.message)) {
                                for (var i in qq_admin_list) {
                                    if (req.body.user_id == qq_admin_list[i]) {
                                        var msg_9 = req.body.message.replace("/admin_change_reply_probability ", "");
                                        reply_probability = msg_9;
                                        res.send({
                                            reply: "\u5C0F\u591C\u56DE\u590D\u7387\u5DF2\u4FEE\u6539\u4E3A" + msg_9 + "%"
                                        });
                                        return 0;
                                    }
                                }
                            ...
                            if (chaos_flag < chaos_probability) {
                                ...
                                PrprDoge()
                                    ...
                                    RandomGroupList()
                                        .then(function (resolve) {
                                        request("http://" + go_cqhttp_api + "/send_group_msg?group_id=" + resolve + "&message=" + encodeURI(prprmsg_1), function (error, _response, _body) {
                                            if (!error) {
                                                console.log(("qqBot\u5C0F\u591C\u5728\u7FA4 " + resolve + " \u62BD\u98CE\u4E86\uFF0C\u53D1\u9001\u4E86 " + prprmsg_1).log);
...
function LoopDanmu() {
    GetLaststDanmu()
        ...
        if (last_danmu_timeline === resolve.timeline) {
            ...
            if (ditupao_flag < 1) {
                ChatProcess("").then(function (resolve) {
                    var reply = resolve;
...
exports.Curentyyyymmdd = Curentyyyymmdd;
//时分秒
function CurentTime() {
    var now = new Date();
    var hh = now.getHours();
        ...
        request("https://api.sumt.cn/api/rand.tbimg.php?token=" + sumtkey + "&format=json", function (err, response, body) {
            ...
            if (!err && body.code === 200) {
                ...
                request(picUrl_1).pipe(fs.createWriteStream("./static/images/" + picUrl_1.split("/").pop()).on("close", function (_err) {
                    console.log(("\u4FDD\u5B58\u4E86\u73CD\u8D35\u7684\u968F\u673A\u4E70\u5BB6\u79C0\uFF1A" + picUrl_1 + "\uFF0C\u7136\u540E\u518D\u7ED9\u7528\u6237").log);
                })); //来之不易啊，保存为本地图片
...
exports.RandomTbshow = RandomTbshow;
//随机二次元图
function RandomECY() {
    return new Promise(function (resolve, reject) {
        request("https://iw233.cn/api/Random.php", function (err, response, _body) {
            if (!err) {
                ...
                request(picUrl_2).pipe(fs.createWriteStream("./static/images/" + picUrl_2.split("/").pop()).on("close", function (_err) {
                    console.log(("\u4FDD\u5B58\u4E86\u597D\u5EB7\u7684\u4E8C\u6B21\u5143\u56FE\uFF1A" + picUrl_2 + "\uFF0C\u7136\u540E\u518D\u7ED9\u7528\u6237").log);
                })); //来之不易啊，保存为本地图片
...
exports.RandomHomeword = RandomHomeword;
//自动随机昵称
...
exports.sqliteAll = sqliteAll;
//异步结巴 by@ssp97
function ChatJiebaFuzzy(msg) {
    return __awaiter(this, void 0, void 0, function () {
        var candidate, candidateNextList, candidateNextGrand, _a, _b, _i, key, element, k, answer, key, element;
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 1:
                    ...
                    element = msg[key];
                    console.log(element);
                    return [4 /*yield*/, sqliteAll("SELECT * FROM chat WHERE ask LIKE '%" + element.word + "%'")];
                case 2:
                    rows = _c.sent();
                    console.log(rows);
                    for (k in rows) {
...
exports.ChatJiebaFuzzy = ChatJiebaFuzzy;
...
exports.ChatProcess = ChatProcess;
//保存qq侧传来的图
...
exports.SaveQQimg = SaveQQimg;
//随机选取一个群
function RandomGroupList() {
    return new Promise(function (resolve, reject) {
        request("http://" + go_cqhttp_api + "/get_group_list", function (err, response, body) {
            body = JSON.parse(body);
            if (!err && body.data.length != 0) {
...
exports.RandomGroupList = RandomGroupList;
//获取balabala
...
exports.GetBalabalaList = GetBalabalaList;
//语音合成TTS
function TTS(tex) {
    return new Promise(function (resolve, reject) {
        if (!tex)
        ...
        SpeechClient.text2audio(tex, {
            ...
            if (result.data) {
                ...
                var file = { file: "/xiaoye/tts/" + sha1(result.data) + ".mp3", filename: "小夜语音回复" };
                resolve(file);
            }
            else {
                // 合成服务发生错误
    ...
    return new Promise(function (resolve, reject) {
        ...
        request("https://ai.baidu.com/aidemo?type=tns&per=4103&spd=6&pit=10&vol=10&aue=6&tex=" + encodeURI(tex), function (err, _response, body) {
            ...
            if (!err && body.data) {
                ...
                var file = { file: "/xiaoye/tts/" + sha1(dataBuffer) + ".mp3", filename: "小夜幼女版语音回复" };
                resolve(file);
            }
            else {
                //估计被发现扒接口了
...
exports.GetLaststDanmu = GetLaststDanmu;
//随机延时提醒闭菊的群
function DelayAlert(service_stoped_list) {
    ...
    var _loop_1 = function (i) {
        ...
        setTimeout(function () {
            request("http://" + go_cqhttp_api + "/send_group_msg?group_id=" + service_stoped_list[i] + "&message=" + encodeURI(random_alert_msg), function (error, _response, _body) {
                if (!error) {
...
function Gugua(who) {
    ...
    var _loop_2 = function (i) {
        var file_online = "http://127.0.0.1:" + web_port + "/xiaoye/ps/" + gugua_pic_list[i];
...
exports.QunGugua = QunGugua;
//百科问答题库
function WenDa() {
    return new Promise(function (resolve, reject) {
        request("http://api.tianapi.com/txapi/wenda/index?key=" + Tiankey, function (err, response, body) {
            body = JSON.parse(body);
            if (!err) {
...
exports.ECYWenDa = ECYWenDa;
//彩虹屁回复
function RainbowPi() {
    return new Promise(function (resolve, reject) {
        request("http://api.tianapi.com/txapi/caihongpi/index?key=" + Tiankey, function (err, response, body) {
            body = JSON.parse(body);
            if (!err) {
                resolve(body.newslist[0].content);
            }
